...
async def novo_preco(message: Message, state: FSMContext):
    ...
    texto_grupo = (
        f"🔒 <b>Prognóstico</b>\n"
        f"🏆 {html_utils.escape(dados.get('liga', ''))}\n"
        f"📅 {html_utils.escape(dados.get('data_hora_jogo', ''))}\n"
        f"🎯 Mercado: {html_utils.escape(dados.get('mercado', ''))}\n"
...
async def callback_desbloquear(callback: CallbackQuery):
    ...
    nome_escapado = html_utils.escape(callback.from_user.full_name)
    username_escapado = html_utils.escape(callback.from_user.username or "sem username")
    liga_escapada = html_utils.escape(prog["liga"] or "")
    await bot.send_message(
        ADMIN_ID,
        f"💰 <b>Pedido de desbloqueio</b>\n"
        f"Utilizador: {nome_escapado} (@{username_escapado})\n"
        f"Prognóstico: #{prognostico_id} — {liga_escapada}\n"
...
async def cmd_pendentes(message: Message):
    ...
    for p in pendentes:
        liga_txt = html_utils.escape(p["liga"] or "sem liga")
        data_txt = html_utils.escape(p["data_hora_jogo"] or "sem data")
        linhas.append(
            f"#{p['id']} — {liga_txt} | Odd: {p['odd_betano']} | {data_txt}"
        )
...
async def cmd_todos(message: Message):
    ...
    for p in registos:
        ...
        data_txt = html_utils.escape(p["data_hora_jogo"] or "sem data")
        linhas.append(
            f"{e} #{p['id']} — {liga_txt} | Odd: {p['odd_betano']} | {data_txt}"
        )
...
async def processar_resultado(message: Message, texto_comando: str, foto_file_id: str = None):
    ...
    if mensagem_id and prog:
        ...
        texto_atualizado = (
            f"🔒 <b>Prognóstico</b>\n"
            f"🏆 {html_utils.escape(prog['liga'] or '')}\n"
            f"📅 {html_utils.escape(prog['data_hora_jogo'] or '')}\n"
            f"🎯 Mercado: {html_utils.escape(prog['mercado'] or '')}\n"
